...
var project_folder = require("path").basename(__dirname);
...
function js() {
  return src(path.src.js).pipe(fileinclude()).pipe(dest(path.build.js)).pipe(uglify()).pipe(rename({
    extname: ".min.js"
  })).pipe(dest(path.build.js)).pipe(browsersync.stream()); //    .pipe(dest(path.build.js)) это функция копирования и вставки файла
}
...
gulp.task('svgSprite', function () {
  return gulp.src([source_folder + '/iconsprite/*.svg']).pipe(svgSprite({
    mode: {
      stack: {
...
function cb() {} //Функция слежки за файлами
...
var build = gulp.series(clean, gulp.parallel(js, css, html, images, fonts), fontsStyle);
...
exports.build = build;
exports.watch = watch;
exports["default"] = watch; //остановился на создании scss файлов 01:10:39 сек